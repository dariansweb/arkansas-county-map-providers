  ...
  fetch("data/cbp-region-summary.json")
    ...
    .then((data) => {
      ...
      data.forEach((cbp) => {
        ...
        const countyBadges = cbp.counties_served
          .map((county) => `<span class="badge">${county}</span>`)
          .join(" ");
        ...
        card.innerHTML = `
            ...
            <ul>
              ...
              <li><strong>Avg Services (2020–2024):</strong> ${
                cbp.avg_services
              }</li>
              <li><strong>Per 1k Youth:</strong> ${
                cbp.services_per_1000_youth
              }</li>
            </ul>
      ...
      fetch("data/cbp-sparklines.json")
        ...
        .then((sparklines) => {
          Object.entries(sparklines).forEach(([provider, data]) => {
            const card = [...document.querySelectorAll(".cbp-card")].find(
              (c) => c.querySelector("h3")?.textContent === provider
            );
...
function drawSparkline(container, data) {
  const width = 200;
  const height = 40;
  const margin = { top: 12, right: 4, bottom: 4, left: 4 };
  ...
  const y = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.services)])
    .nice()
  ...
  svg
    ...
    .append("text")
    .attr("x", (d) => x(d.year))
    .attr("y", (d) => y(d.services) - 8)
    .text((d) => d.year)
    .attr("text-anchor", "middle")
    .attr("font-size", "9px") // Increased from 7px to 9px
    .attr("font-weight", "bold") // Now bold!
    .attr("fill", "#222");
}
